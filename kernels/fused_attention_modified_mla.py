...
def _attn_fwd(
    ...
    start_m = tl.program_id(0) # Index along N_CTX 
    off_hb = tl.program_id(1)  # B*NumHeads index

    off_b = off_hb // H # Batch we are in
    off_h = off_hb % H # Head we are in

    # Calculate base pointers for K, V, O 
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale):
        # Input shape: (B, H, N_CTX, HEAD_DIM) 
        q = q.contiguous()
        k = k.contiguous()
        v = v.contiguous()
        ...
        # M tensor for storing softmax max values
